...
export async function POST(req: NextRequest) {
  try {
    ...
    for (let i = 0; i < quantity; i++) {
      ...
      keyRows.push({ encrypted_key: encryptedKey, commitment_hash: commitmentHash });
    ...
    return NextResponse.json({ success: true, count: keys.length, keys, batchId, totalAvailable });
  } catch (error: any) {
