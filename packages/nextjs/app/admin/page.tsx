...
export default function AdminPage() {
  ...
  return (
    <div className="flex items-center flex-col grow pt-10 px-5 max-w-3xl mx-auto">
      ...
      <p className="text-base-content/70 mb-8">Manage SoulKey game contracts and CD key batches</p>

      <div className="alert alert-success mb-6 w-full">
        <span>✅ Owner: {connectedAddress}</span>
        {VAULT_ADDRESS && <span className="font-mono text-xs">📄 Vault: {VAULT_ADDRESS}</span>}
      </div>
            ...
            {(!contractStatus.inDB || !contractStatus.metadataCid) && (
              <div className="form-control">
                <label className="label">
                  ...
                  <span className="label-text-alt">Game name, image & description are fetched automatically</span>
                </label>
                <input
              ...
              <table className="table table-xs w-full">
                ...
                <tbody>
                  {keys.map((key, i) => (
                    <tr key={i}>
                      <td>{i + 1}</td>
                      <td className="font-mono text-xs truncate max-w-xs">{key.commitmentHash}</td>
                      <td>
                        <button
